#!/usr/bin/env python3
"""Hermes cron: Generate 5 new comparison articles in underserved categories via Gemini."""
import json
...
SITE_ROOT = Path("/home/cls/kieskeuken")
...
{{
  ...
  "featuredProduct": "Naam beste product (kan combinatie zijn zoals 'Heggenschaar (kleine tuin) / Bosmaaier (groot terrein)')",
  "priceRange": "EUR X-Y",
...
def build_md(spec, data, slug):
    ...
    # Related slugs — pick from same category
    related_map = {
...
for spec in SPECS:
    ...
    try:
        ...
        results.append({"slug": slug, "status": "ok", "bytes": len(article)})
    except json.JSONDecodeError as e:
        print(f"JSON ERROR: {e}")
        Path(f"/tmp/{slug}-raw.txt").write_text(raw)
        print(f"  Raw saved to /tmp/{slug}-raw.txt")
        results.append({"slug": slug, "status": "json_error", "error": str(e)})
