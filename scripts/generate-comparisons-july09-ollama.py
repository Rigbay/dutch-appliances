#!/usr/bin/env python3
"""Generate remaining 4 comparison articles using Ollama (Gemini free tier exhausted).
...
2. Snel advies — 3 bullets wie wat moet kiezen: starter (set), ervaren kok (los), budgetbewust (set)
...
- Prijzen in euro's
- Schrijf ALLEEN de Markdown body, geen frontmatter, geen ``` markers""",
        "product_names": ["Tefal Ingenio Pannenset 10-delig", "BK Pannenset 5-delig", "GreenPan Pannenset Memphis", "BK Koekenpan 28cm", "Tefal Steelpan 16cm", "GreenPan Braadpan 24cm"],
        "product_links": [f"https://www.amazon.nl/s?k=Tefal+Ingenio+pannenset+10+delig&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=BK+pannenset+5+delig&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=GreenPan+pannenset+Memphis&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=BK+koekenpan+28cm&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Tefal+steelpan+16cm&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=GreenPan+braadpan+24cm&tag={AMAZON_TAG}"],
...
- Schrijf ALLEEN de Markdown body, geen frontmatter, geen ``` markers""",
        "product_names": ["Stoov Big Hug Elektrische Deken", "Inventum Elektrische Deken HN131", "Medisana Elektrische Onderdeken", "Eurom Infraroodpaneel 400W", "Mill Infraroodpaneel 600W", "Klarstein Infraroodpaneel 800W"],
        "product_links": [f"https://www.amazon.nl/s?k=Stoov+Big+Hug+elektrische+deken&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Inventum+elektrische+deken+HN131&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Medisana+elektrische+onderdeken&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Eurom+infraroodpaneel+400W&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Mill+infraroodpaneel+600W&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Klarstein+infraroodpaneel+800W&tag={AMAZON_TAG}"],
...
- Prijzen in euro's
- Schrijf ALLEEN de Markdown body, geen frontmatter, geen ``` markers""",
        "product_names": ["Felco 2 Snoeischaar", "Gardena Bypass Snoeischaar", "Fiskars PowerGear Snoeischaar", "Bosch Heggenschaar EasyHedgeCut 18V", "Gardena Heggenschaar Accu 18V", "Black+Decker Heggenschaar 600W"],
        "product_links": [f"https://www.amazon.nl/s?k=Felco+2+snoeischaar&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Gardena+bypass+snoeischaar&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Fiskars+PowerGear+snoeischaar&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Bosch+EasyHedgeCut+18V+heggenschaar&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Gardena+heggenschaar+accu+18V&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Black+Decker+heggenschaar+600W&tag={AMAZON_TAG}"],
...
- Prijzen in euro's
- Schrijf ALLEEN de Markdown body, geen frontmatter, geen ``` markers""",
        "product_names": ["Severin Yoghurtmaker JG3521", "Rommelsbacher Yoghurtmaker JG80", "Tefal Yoghurtmaker YG654", "DIY Yoghurt met Ovenlamp", "DIY Yoghurt met Thermoskan", "DIY Yoghurt met Warmhoudpan"],
        "product_links": [f"https://www.amazon.nl/s?k=Severin+yoghurtmaker+JG3521&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Rommelsbacher+yoghurtmaker+JG80&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=Tefal+yoghurtmaker+YG654&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=yoghurt+maken+startpakket&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=thermoskan+1+liter&tag={AMAZON_TAG}", f"https://www.amazon.nl/s?k=yoghurt+culturen+starter&tag={AMAZON_TAG}"],
        "related": ["beste-yoghurtmaker-2026", "beste-keukenmachine-2026", "beste-slowcooker-2026"]
    },
]


def call_ollama(prompt_text, model="qwen3:14b"):
    """Call Ollama API."""
    body = json.dumps({
        "model": model,
        "prompt": prompt_text,
...
def main():
    ...
    for topic in TOPICS:
        ...
        t0 = time.time()
        try:
            body_md = call_ollama(topic["prompt"])
