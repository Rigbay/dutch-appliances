#!/usr/bin/env python3
"""Enrich thin articles (<300 words body) using Gemini API. July 2026 cron."""
...
API_KEY = ""
# Read from .env file directly (more reliable than env vars which may have placeholders)
...
if not API_KEY:
    ...
    if env_val and "your_gemini" not in env_val.lower():
        API_KEY = env_val
...
def extract_frontmatter_and_body(filepath: str) -> tuple:
    """Extract frontmatter dict and body text from a markdown file."""
    ...
    # Parse frontmatter into dict (simple YAML-like)
    fm = {}
...
def build_enrichment_prompt(fm: dict, existing_body: str, filename: str) -> str:
    """Build a prompt for Gemini to generate proper article body."""
    ...
    products_text = ""
    # We'll parse products from the raw frontmatter
...
def main():
    ...
    for f in sorted(os.listdir(REVIEWS_DIR)):
        ...
        if words < 300:
            thin_articles.append((f, words, fm, body))
    
    print(f"Found {len(thin_articles)} thin articles (<300 words body)")
    
    enriched = []
    failed = []
    
    for filename, word_count, fm, body in thin_articles:
        ...
        # Reconstruct the file: frontmatter + new body
