#!/usr/bin/env python3
"""Generate 5 high-impact comparison articles for July 2026 using Gemini 2.5 Flash-Lite.
...
# 5 high-value comparison gaps — none exist yet
TOPICS = [
    {
...
- Prijzen in euro's""",
    ...
    },
    {
        ...
        "category": "keuken",
        "prompt": f"""Je bent een Nederlandse consumentenjournalist. Schrijf een complete koopgids in het Nederlands over: Koffiezetapparaat vs French Press (Cafetière) 2026.

Context: Juli 2026. Nederlanders zijn koffieland nummer 1 in Europa qua consumptie per hoofd. De keuze tussen een filterkoffiezetapparaat (gemak, timer, grote kan) en een French press/cafetière (volle smaak, handwerk, goedkoop) is actueel.

STRUCTUUR:
1. Inleiding (2-3 alinea's) — filterkoffie vs French press: gemak vs smaakbeleving
2. Snel advies — wie kiest wat (druk gezin, koffieliefhebber, budget)
3. Hoe werkt het? — filterkoffie (heet water door papieren filter) vs French press (koffie trekt in heet water, plunger scheidt)
4. Vergelijkingstabel — 6 producten: Product | Type | Capaciteit | Timer | Prijs | Score
5. Diepere vergelijking: smaakprofiel, zetgemak, schoonmaak, kosten per kopje, duurzaamheid (papieren filters vs geen afval)
6. Conclusie

BELANGRIJK:
- Amazon NL links met tag={AMAZON_TAG}
- Products: Philips Grind & Brew Koffiezetapparaat, Moccamaster KBG Select, Bodum Chambord French Press, Melitta AromaFresh, Bialetti French Press, Russell Hobbs Koffiezetapparaat
- Minimaal 1500 woorden
- Eerlijk over minpunten (French press: koffiedik, geen timer; filterapparaat: papieren filters, minder volle smaak)
- Prijzen in euro's""",
        "product_names": [
            "Philips Grind & Brew Koffiezetapparaat HD7762/00",
            "Moccamaster KBG Select Filterkoffiezetapparaat",
            "Bodum Chambord French Press 1L",
            "Melitta AromaFresh Koffiezetapparaat",
            "Bialetti French Press 1L",
            "Russell Hobbs Koffiezetapparaat 22000-56"
        ],
        "product_links": [
            f"https://www.amazon.nl/s?k=Philips+Grind+Brew+HD7762&tag={AMAZON_TAG}",
            f"https://www.amazon.nl/s?k=Moccamaster+KBG+Select&tag={AMAZON_TAG}",
            f"https://www.amazon.nl/s?k=Bodum+Chambord+French+Press&tag={AMAZON_TAG}",
            f"https://www.amazon.nl/s?k=Melitta+AromaFresh+koffiezetapparaat&tag={AMAZON_TAG}",
            f"https://www.amazon.nl/s?k=Bialetti+French+Press&tag={AMAZON_TAG}",
            f"https://www.amazon.nl/s?k=Russell+Hobbs+koffiezetapparaat+22000&tag={AMAZON_TAG}"
        ],
        "related": ["beste-koffiezetapparaat-2026", "koffiemachine-bonen-vs-cups-2026", "espresso-vs-filterkoffie-2026"]
    },
    {
...
def main():
    ...
    if generated > 0:
        print(f"To deploy: git add src/content/reviews/*.md && git commit -m 'Add July comparison articles' && git push")
